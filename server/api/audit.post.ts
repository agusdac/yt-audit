...
export default defineEventHandler(async (event) => {
  ...
  const uniqueHandles = [...new Set(body.handles.map(normalizeHandle).filter(Boolean))]

  const allVideos: VideoDetails[] = []

  for (const handle of uniqueHandles) {
    try {
      const playlistId = await getUploadsPlaylistId(handle, config.ytApiKey)
      ...
      throw createError({ statusCode: 500, message: `Failed to fetch channel @${handle}: ${msg}` })
    }
  }
