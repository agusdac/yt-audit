<template>
  <div class="p-6 max-w-4xl mx-auto">
    ...
    <template v-else>
      <div v-if="tierRef === 'free'" class="rounded-card p-8 bg-card-bg-attention border border-border-default mb-6">
        <h1 class="text-2xl font-bold text-text-primary mb-2">High-Intent Comments</h1>
        <p class="text-text-muted mb-4">
          Find purchase-intent comments across your videos—where to buy, discount codes, and more. This is a Pro
          feature.
        </p>
        <NuxtLink to="/settings"
          class="inline-flex px-6 py-3 rounded-button font-semibold bg-gradient-to-r from-btn-from to-btn-to text-white hover:from-btn-hover-from hover:to-btn-hover-to">
          Upgrade to Pro
        </NuxtLink>
      </div>
      ...
      <div class="flex items-center justify-between flex-wrap gap-4 mb-6">
        ...
        <button type="button"
          ...
          :disabled="store.isFetchingComments || store.videos.length === 0" @click="store.fetchComments">
          {{ store.isFetchingComments ? 'Fetching...' : 'Fetch comments' }}
        </button>
      </div>

      <p class="text-text-muted mb-6">
        Find comments that ask where to buy, discount codes, or show purchase intent. These viewers could become
        customers.
      </p>
      ...
      <div v-else-if="store.highIntentComments.length > 0" class="space-y-4">
        <div class="flex items-center justify-between flex-wrap gap-3">
          <div class="flex items-center gap-3 flex-wrap">
            ...
            <button type="button" class="px-3 py-1.5 rounded-button text-sm font-medium transition-all border"
            ...
            <button v-if="tierRef === 'pro' && selectedReplyableCount > 0" type="button"
              ...
              :disabled="replying" @click="replyModalOpen = true">
          ...
          <button type="button"
            ...
            :class="{ 'border-merch-border text-merch-link': exportSuccess }" @click="exportToCsv">
            {{ exportSuccess ? 'Exported!' : 'Export to CSV' }}
          </button>
        </div>
        <div v-for="c in displayedComments" :key="c.id"
          ...
          <div v-if="tierRef === 'pro'" class="flex-shrink-0">
            <input :id="`sel-${c.id}`" type="checkbox" :checked="selectedIds.has(c.id)" :disabled="c.canReply === false"
          ...
          <div class="flex flex-shrink-0 gap-2">
            <button type="button" class="px-4 py-2 rounded-button text-sm font-medium transition-colors"
            ...
            <button v-if="tierRef === 'pro'" type="button"
        ...
        <Teleport to="body">
          <div v-if="replyModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50"
            @click.self="replyModalOpen = false">
            <div
              class="rounded-card p-6 bg-card-bg border border-border-default max-w-lg w-full max-h-[90vh] overflow-y-auto">
              <h3 class="text-lg font-semibold text-text-primary mb-3">Reply to {{ selectedCount }} comment(s)</h3>
              <p class="text-sm text-text-muted mb-3">Use <code
                  class="px-1 py-0.5 rounded bg-card-bg-attention">&#123;&#123;author&#125;&#125;</code> to insert the
                commenter's name.</p>
              <textarea v-model="replyTemplate" rows="4" placeholder="Thanks {{author}}! Here's the link: ..."
              ...
              <div v-if="replyError"
                class="rounded-card px-4 py-3 bg-error-bg border border-error-border text-error-text mb-3 flex flex-wrap items-center gap-3">
                <span class="flex-1 min-w-0">{{ replyError }}</span>
                <NuxtLink v-if="isReplyProRequiredError" to="/settings"
              ...
              <div class="flex gap-3">
                <button type="button"
                  ...
                  :disabled="replying || !replyTemplate.trim()" @click="sendBulkReplies">
                  {{ replying ? 'Replying...' : 'Send' }}
                </button>
                <button type="button"
                  class="px-4 py-2 rounded-button text-sm font-medium bg-card-bg border border-border-default text-text-primary hover:bg-card-bg-attention"
                  :disabled="replying" @click="replyModalOpen = false">
                  Cancel
                </button>
              </div>
            </div>
      ...
      <div v-else-if="store.hasVideos" class="rounded-card p-8 bg-card-bg border border-border-default text-center">
        ...
        <button type="button"
          ...
          :disabled="store.isFetchingComments" @click="store.fetchComments">
          Fetch comments
        </button>
      </div>
...
const sendBulkReplies = async () => {
  ...
  try {
    ...
    if (res.replied > 0 && store.me?.linkedChannels?.length) {
