  ...
  <div class="min-h-screen font-sans bg-page-bg text-text-primary">
    <header class="relative overflow-hidden border-b border-border-default">
      ...
      <div class="relative max-w-4xl mx-auto px-6 py-16 text-center">
        ...
        <button
          type="button"
          class="px-8 py-4 rounded-button font-semibold text-lg transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed bg-gradient-to-r from-btn-from to-btn-to shadow-btn hover:from-btn-hover-from hover:to-btn-hover-to hover:scale-[1.02] active:scale-[0.98]"
          :disabled="isLoading"
          @click="runAudit"
        >
    ...
    <div v-else-if="!error" class="max-w-xl mx-auto px-6 py-20 text-center">
      <p class="text-lg text-text-muted">
        Click <strong class="text-text-primary">Run Audit</strong> to scan your videos.
      </p>
    </div>
  </div>
</template>
...
const HARDCODED_PLAYLIST_ID = 'UUrPseYLGpNygVi34QpGNqpA'
...
const runAudit = async () => {
  ...
  try {
    const response = await $fetch<{ count: number; videos: VideoDetails[] }>('/api/video-details', {
      query: { playlistId: HARDCODED_PLAYLIST_ID }
    })
