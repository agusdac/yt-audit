<template>
  <div v-if="items.length > 0" class="rounded-card overflow-hidden"
    ...
    <button type="button"
      ...
      @click="open = !open">
      <h3 class="font-bold text-lg" :class="variant === 'dead' ? 'text-error-text' : 'text-alert-text'">
        {{ title }}
      </h3>
      <span class="text-sm" :class="variant === 'dead' ? 'text-error-text/80' : 'text-alert-text/80'">
        {{ items.length }} link{{ items.length > 1 ? 's' : '' }}
      </span>
      <span class="text-xl transition-transform"
    ...
    <div v-show="open" class="p-6 pt-0 max-h-[320px] overflow-y-auto scroll-area">
      <div class="space-y-4">
        <div v-for="item in sortedItems" :key="item.url" class="rounded-lg p-4 bg-card-bg space-y-3"
          :class="variant === 'dead' ? 'border border-error-border' : 'border border-alert-border'">
          <p class="text-sm break-all font-mono"
...
const sortedItems = computed(() =>
  [...props.items].sort((a, b) => (b.revenueLoss ?? 0) - (a.revenueLoss ?? 0))
)
</script>
